...
- 本文の最後に「#### 💡 注目ポイント」の見出しで、開示事実の範囲で投資家が見落としやすい観点
  ...
  **意見・評価・推奨・将来予測は書かない**（事実の対比と開示済み説明の指摘に徹する）。材料が無ければこの節は省略。
...
AUDIENCE_STYLES: dict[str, str] = {
    "beginner": (
        "読者は投資をはじめたばかりの個人。専門用語には初出時に一言のやさしい注釈を付ける"
        "（例:「営業利益（本業の儲け）」「前年比（前の年と比べた増減）」）。"
        "身近なたとえを1つまで使ってよい。一文は短く区切り、段落も短めに。"
        "ただし分析の内容・数値・専門性そのものは絶対に薄めない。"
    ),
    "intermediate": (
    ...
    "advanced": (
        "読者は経験豊富な投資家。注釈・前置き・言い換えは不要。専門用語をそのまま使い、"
        "密度高く簡潔に。数値の含意・事業構造への示唆・会社説明との整合を中心に述べる。"
    ),
}
...
def _write_stream(name, query, facts_ctx, passages_ctx, focus_metrics, audience: str):
