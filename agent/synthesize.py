...
- relevant_metrics には、回答で触れた**主要指標**の metric_key を入れる（散文の数値に対応するカードを表示するため）。
  業績全般など広い質問でも**網羅せず中心指標に絞る**（カードは要点用。セグメント詳細や全期間の数表はprose内の表で示す）。
...
def synthesize(query: str, company: dict[str, Any]) -> dict[str, Any]:
    ...
    # カード過多の抑制（広い質問対策）。上限超過時のみ縮約。狭い質問は無傷。
