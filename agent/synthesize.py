...
# can_answer の判断（重要・上から順に適用）
- **最優先**: 「開示資料の抜粋（FAQ含む）」の中に、質問に**直接答えている**記述があれば → **必ず can_answer=true**。
  used_citations にその番号を入れる。これは利用可能な構造化数値リストに無い指標（例: ROE）でも適用する
  （IR承認済みFAQ／開示資料に答えがある限り、エスカレーションしない）。relevant_metrics は該当する構造化指標
  があれば入れ、無ければ空配列でよい（その場合は引用だけで接地する）。
...
- **can_answer=false にするのは次だけ**：利用可能な構造化数値リストにも開示抜粋(FAQ含む)にも答えが無い指標（例: 抜粋に
  無い ROE）／長期・過去◯年の推移が未開示／未開示の重要情報／将来予測。質問の制約（「数値で」等）を利用可能データで
  満たせない場合も false。**ただし上の「最優先」に該当するなら必ず true。**
