 ...
 * 例: 次質問サジェスト / 「公式Q&A N件をみる」/ 「＋ 新規追加」。
 */
export const PILL_INK = `${PILL_BASE} ${PILL_LIFT} border-ink bg-paper px-3.5 py-1.5 text-[11.5px] text-ink hover:bg-ink hover:text-cream`;

/** 主アクションのピル（大）。並べて主役に見せたいとき（次質問サジェスト）。 */
export const PILL_INK_MD = `${PILL_BASE} ${PILL_LIFT} border-ink bg-paper px-3.5 py-2 text-xs text-ink hover:bg-ink hover:text-cream`;
