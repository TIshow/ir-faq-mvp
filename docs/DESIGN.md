...
## コンセプト: クリーム × インク × ポップ ＝「ポップエディトリアル」
**脱・金融感**。黒背景×数字の羅列という「いかにも金融」ではなく、**凄腕の金融クリエーターが編集した誌面**のトーン。
- **クリーム（地）× インク（文字）**: 紙のような温かい下地に濃いインク＝雑誌・エディトリアルの読み心地。
- **ポップ（差し色）**: グリーン=気づき・成長、黄=マーカー・会社予想、コーラル=注意。大きな数字（Outfit）・マーカー強調・丸ゴシック見出しで「読ませる」。
- **やさしいが、軽くない**: 見た目はフレンドリーでも、中身は決定論の数値と出典（§7の原則）。翻訳するのは言葉とトーンだけで、専門性は薄めない——読者レベル（カジュアル/スタンダード）と同じ思想。
迷ったら「誌面ならどう組むか？」で判断する。
...
| `IR Admin Dashboard.dc.html` | IR管理画面（/ir）の刷新案 | **採用・実装済み**（KPI4枚＋2カラム＋週次チャート。デザイン内の"インサイト文"は決定論ソースが無いため見送り） |
...
- **吹き出しガーデン**（初期画面・`ChatInterface.tsx`）: よく聞かれる質問を吹き出しで散らす。**順位（index）で大きさ・色・角の落とし方・上限幅・縦の段差がすべて決まる＝決定論**（1位=インク面で最大＋「いちばん聞かれています」／2位=グリーン／3位=白／4位=イエロー／以降は小さく、末尾は破線）。デザイン（`Naruhodo IR Home`）はカードを回転させているが**実装では回さない**（可読性優先）。代わりに幅と縦位置の段差で散らし、段差は `sm` 以上だけ（スマホは1列に積むので段差があると崩れて見える）。日本語の行割れは `text-balance` で整える。**タップで即送信**（下書きに入れるワンクッションは置かない＝リズムを切らない）。**件数（「N人が質問」）は出さない**——会話の本文をどこにも保存していないため質問単位の集計が存在せず、書けば捏造になる（原則2）。
- **公式Q&Aパネル**（`QaPanel.tsx`・#113）: デスクトップは右に生えて**二画面**（幅をアニメーション）、スマホは右から**全面カバー**。**常時DOMに描画し開閉はCSSのみ**＝表示層の都合ではなく機能要件（閉じていてもHTMLに答え全文が載る＝AIクローラーが読める）。閉じている間は `inert` でフォーカスを入れない。Escapeで閉じ、開いたらパネル内へフォーカスを移す。
- **銘柄選択トップ**（`/`）: 会社カード（社名＋ティッカー＋事業＋直近の数字＋Q&A件数）のグリッド。**チャット入力欄は置かない**——横断チャットが動くまで、押せるのに反応しない入力欄は「壊れている」ようにしか見えない。
- **出典チップ**: 「出典：資料名 p.N」のクリームチップ。全カード・参考資料で統一（ディープリンク維持）。
- **ピル（実装の正は `components/ui.ts`）**: 名前があるのに各所で手書きされ、同じ「ピル」で触り心地が3通りに分岐していたため定数へ集約した（#126）。**新しいピルは定数を足す。呼び出し側でクラスを書き足さない。**
  - `PILL_INK` / `PILL_INK_MD`（主アクション）: インク縁の白ピル、ホバーでインク反転＋1px浮く。例=次質問サジェスト（MD）・「公式Q&A N件をみる」・「＋ 新規追加」。
  - `PILL_QUIET`（副次アクション）: 淡い縁、ホバーで縁と文字がインクへ。**浮かない**——画面から降りる操作に前へ出る手触りを付けない。例=「閉じる」「新しいチャット」。
  - `/ir` の `BTN_POP` / `BTN_CORAL` / `FIELD_CREAM` はダッシュボード専用の別系統（塗り・コーラル・入力欄）。役割が違うので統合しない。
- **IRダッシュボード（`/ir`）**: 投資家側と同じトークンで統一（#111）。KPIカード（総質問数は**インク地**＋前期間比±・自動回答率は緑＋プログレスバー・IR要対応は「要チェック！」の傾きバッジ=件数>0のみ）／話題トレンドはタクソノミー別アイコン＋色分けバー（対象外=コーラル/その他=ミュート/通常=緑）／週次は棒グラフ（最新週=緑）。ピル/入力欄は `BTN_POP`/`BTN_CORAL`/`FIELD_CREAM` の部品定数（`app/ir/page.tsx`）。**"インサイト文"は載せない**（決定論ソースが無い＝原則2）。

## 6. モーション原則
- 種類は **transform / opacity のみ**（GPUコンポジタ・レイアウト再計算なし）。無限ループさせない（回答表示時に一度だけ）。
- キーフレーム: `fade-slide-in`（出現）/ `pop-in`（バッジ・芽）/ `bar-grow`（棒）/ `stem-grow`・`twig-grow`（蔦）/ `caret-blink`（執筆中）/ `bubble-float`（吹き出しガーデンの浮遊）。
  ※ `bubble-float` は初期画面が「生きている」感を出すための**唯一の無限ループ例外**（振幅6pxのみ・reduced-motion で静止）。
