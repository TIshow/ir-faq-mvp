...
GitHub: https://github.com/TIshow/ir-faq-mvp （main、PR #1〜#117 マージ済）。Issue: #3 経緯と残課題 / #42 FAQサジェスト(A本実装) / #46 IRインテリジェンス epic / #67 派生指標Phase2(CAGR・ROE/ROIC=B1データ投入待ち) / **#77 戦略（足りないもの・moat・残タスクTier）** / #85-87 尖らせ方(MCPエンドポイント・話題フォロー・フェデレーション) / **#88-92 インフラ（#88 ir-agent非公開化=✅完了、#89 BQ東京=データ空の今が好機、#90 SA分離、#91 モデル世代管理、#92 小規模ハードニング）** / **#97 Tier A（ハークスレイ実トラフィックで複利ループ1周）** / **#98 B1（層1縦深化: 多年度+BS/CF→ROE/ROIC解禁）** / **#107 層2精度（評価セット50〜100問＋取り込み方式比較: raw/digital/layout/手動MD/XBRL併用）** / **#113 AIに引用させる（第1弾=PR #117 マージ済。段階Cが残）** / **#118 SSEのエラー応答から例外メッセージを外す（`py/stack-trace-exposure`）**。
...
### Tier 1.2 — AIに引用させる（#113・GEO）
- ✅ **1.2-1 銘柄URL＋公式Q&Aパネル（段階B）** = 実装済み（PR #117）。層1由来の決定論Q&A・常時DOM描画・JSON-LD・robots/sitemap/llms.txt。
- **1.2-2 段階C: 人気順の並べ替え** — `interactions.topic`（BQ・話題別件数）で吹き出しと公式Q&Aを実績順に。**利用が少ないうちは自動更新しない**方針（企業数・投資家数が増えたら1日1回更新）。現状の並びは `companies.ts` の `guidedQuestions`（＝IR/我々の判断）で、実測ではない。
- **1.2-3 段階C: 層2のFAQをパネルに載せる** — いまは層1（数値）由来のみ。想定問答（faq.csv）が入れば定性Q&Aも公開対象。
- **1.2-4 効果測定** — AI経由の流入・引用の有無をどう観測するか（Search Console / リファラ / 手動サンプリング）。未着手。
...
> 推奨スタート: セキュリティは ~~#88~~✅完了 → **#89（BQ東京・データが空の今だけ移行ゼロ）**。事業は **#97（Tier A: FAQ投入→ハークスレイで複利ループを1周）**＝堀は実利用でしか育たない（#77）。機能は **#98（B1: 層1の多年度＋BS/CF投入→CAGR/ROE/ROIC解禁。#67が下流）**。品質は **#107（層2の評価セット＋取り込み方式比較＝定性版eval関門の土台）**。gemini-3 は thinking 最小化で先頭〜12s に改善（要観察・重ければ `MODEL_NAME=gemini-2.5-flash` に即戻す）。
