...
> 推奨スタート: セキュリティは **#88（ir-agent非公開化）→ #89（BQ東京・データが空の今だけ移行ゼロ）**。機能は **B1（層1の多年度＋BS/CF投入→CAGR/ROE/ROIC解禁 #67）**。事業は **Tier A（FAQ投入→ハークスレイで複利ループを1周）**＝堀は実利用でしか育たない（#77）。gemini-3 のレイテンシ（先頭〜18s）は要観察＝重ければ `MODEL_NAME=gemini-2.5-flash` に即戻す。
