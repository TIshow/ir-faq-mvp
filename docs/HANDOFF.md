...
| 分析ログ（痛み②） | BigQuery dataset **`ir_analytics`** / table **`interactions`** | `ANALYTICS_ENABLED=1` で記録。Q&A匿名・集計用（PIIなし） |
| IR管理画面 | `/ir`（ダッシュボード）・`/ir/login` | 質問トレンド/エスカレ/FAQ管理。`/api/ir/metrics`(BQ集計)・`/api/ir/faq`(CRUD) |
